...
import { BigMyTask, BigPublicTask } from "../../components/Cards/Task/BigPublicTask";
...
const MySpecificTask = ({ task }:{task:Task}) => {
          ...
          <Box as="main" pt={10} px={18} w="100%">
            <Flex w="100%">
              <Box flex={1} mr={5}>
                ...
                {task ? <BigMyTask task={task} /> : <Spinner />}
                <Heading
                  mb={4}
                  as="h3"
                  fontSize="600"
...
const SpecificTaskPage =  ({taskId})=>{
  ...
  const [task,setTask]:[Task,any] = useState(null)
  useEffect(() => {
    getTask({ taskId })
      .then(({data}) => {
        setTask(data);
  ...
  if(UserStore.isLoggedIn && task.owner ==UserStore.me.id){
    return (<MySpecificTask task={task}/>)
  }
